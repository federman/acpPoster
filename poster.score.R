# Input format Judge poster score poster score poster score ...  Judge name can
# have no spaces.  Tabs or spaces are ok between fields Adjust the variables as
# needed: f <- names of files for each category fnames <- descriptive names for f
# fdir <- directory name assumes that the files have a '.txt' extension judge.min
# <- minimum number of posters a judge must evaluate to allow standardization
...
# f <- c('student.res', 'student.cv') # plain text files with the extension of
# '.txt'
...
poster.score <- function(ifile) {
    ...
    check.counts <- function(melted = df) {
        if (DEBUG > 1)
            print(melted, row.names = FALSE)
        DF <- my.dcast(melted, poster ~ .)
        names(DF) <- c("Poster", "Count")
        cat("POSTERS WITH < 3 EVALUATIONS\n")
        print(DF[(DF[, 2] < poster.min), ], row.names = FALSE)
        DF <- my.dcast(melted, judge ~ .)
        names(DF) <- c("Judge", "Count")
    ...
    # print(cbind(1:top,with(wide.scaled,head(wide.scaled[ order(-scaled.mean,
    # poster) ,], top)[,c(1,ncol(wide.scaled))])),row.names=F)
